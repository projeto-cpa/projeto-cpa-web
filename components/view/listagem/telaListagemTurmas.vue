...
/** API */
import listagemTurma from '../../../api/listagem/listagemTurma.js';
import ativacaoTurma from '../../../api/ativacao/ativacaoTurma.js';
import exclusaoTurma from '../../../api/exclusao/exclusaoTurma.js';
/** ALTERACAO */
import alteracaoTurma from '../../offcanvas/alteracao/alteracaoTurma.vue';
...
export default {
    ...
    data: function () {
        return {
            recebendo: false,
            resultados: [],
            requisicao: listagemTurma,
            retorno: function (resposta) {
                emmiter.emit('aoListarTurma', resposta);
            },
            selecaoTodos: false,
            paginacao: {
                pagina: 0,
    ...
    methods: {
        ...
        excluirItem: async function (item) {
            ...
            if (modal.isConfirmed) {
                ...
                var resposta = await exclusaoTurma(item.id);
        ...
        ativarItem: async function (item) {
            ...
            if (modal.isConfirmed) {
                ...
                var resposta = await ativacaoTurma(item.id);
        ...
        abrirEdicao: function (item) {
            emmiter.emit('abrirEdicaoTurma', item);
        },
    ...
    components: {
        'Filtro': Filtro,
        'Paginacao': Paginacao,
        'alteracaoTurma': alteracaoTurma
    },
    mounted: function () {
        this.recebendo = true;
        emmiter.on('aoListarTurma', this.aoListarTurma);
        emmiter.on('aoAlterarTurma', this.aoAlterarTurma);
