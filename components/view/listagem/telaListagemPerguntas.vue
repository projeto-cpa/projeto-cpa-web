  ...
  data: function () {
    return {
      ...
      retorno: function (resposta) {
        emmiter.emit("aoListarPergunta", resposta);
      },
      selecaoTodos: false,
      paginacao: {
        pagina: 0,
  ...
  methods: {
        ...
        excluirItem: async function (item) {
            ...
            if (modal.isConfirmed) {
                ...
                var resposta = await exclusaoPerg(item.id);
        ...
        abrirEdicao: function (item) {
            emmiter.emit('abrirEdicaoPergunta', item);
        },
        ...
        aoAlterarPergunta: function (item) {
            this.editar = false;
        ...
        aoListarPergunta: function (resposta) {
    ...
    mounted: function () {
        this.recebendo = true;
        emmiter.on('aoListarPergunta', this.aoListarPergunta);
        emmiter.on('aoAlterarPergunta', this.aoAlterarPergunta);
