<script>
// *Script da tela
import scriptTela from '../../../views/cadastro/telaCadastroAvaliacoes/script.js';
...
<template>
    <div class="container-fluid conteudo-principal">
        <section>
            <article>
                <form class="row m-0" ref="formularioCadastro" id="cadastro-avaliacao">
                    <div class="card p-0 card-register">
                        <div class="card-body">
                            <div v-for="campo in formulario" :key="campo.id" :class="campo.classe.coluna">
                                <div class="form-floating">
                                    <template
                                        v-if="campo.tipo !== 'textarea' && campo.tipo !== 'select' && campo.tipo !== 'list'">
                                    ...
                                    <template v-else-if="campo.tipo === 'select'">
                                        <select :placeholder="campo.etiqueta" :name="campo.nome" v-model="campo.valor"
                                            class="form-control" :id="campo.id" @focus="filtrar(campo.valor)" @change="campo.validar()"
                                    ...
                                    <template v-else-if="campo.tipo === 'list'">
                                        <div class="container">
                                            <div class="row">
                                                <div class="col-md-6">
                                                    <table class="table">
                                                        ...
                                                        <tbody>
                                                            <tr v-for="(item, index) in campo.valores" :key="item.id">
                                                                <td>{{ item.id }}</td>
                                                                <td>{{ item.nome }}</td>
                                                                <td class="text-center"><button class="btn btn-primary"
                                                                        @click="adicionarItem(index)">Adicionar</button></td>
                                                            </tr>
                                                        </tbody>
                                                    </table>
                                                </div>
                                                <div class="col-md-6">
                                                    <table class="table">
                                                        ...
                                                        <tbody>
                                                            <tr v-for="(item, index) in listaTurmas" :key="item.id">
                                                                <td>{{ item.id }}</td>
                                                                <td>{{ item.nome }}</td>
                                                                <td class="text-center"><button class="btn btn-primary"
                                                                        @click="removerItem(index)">Remover</button></td>
                                                            </tr>
                                                        </tbody>
                                                    </table>
                                                </div>
